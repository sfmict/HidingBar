...
profilesCombobox:ddSetInitFunc(function(self, level)
	...
	if level == 1 then
		...
		for i, profile in ipairs(hidingBar.profiles) do
			local subInfo = {
				text = profile.isDefault and profile.name.." "..GRAY_FONT_COLOR:WrapTextInColorCode(DEFAULT) or profile.name,
				value = profile.name,
...
barCombobox:ddSetInitFunc(function(self)
	...
	for i, bar in ipairs(main.pBars) do
		local subInfo = {
			text = bar.isDefault and bar.name.." "..GRAY_FONT_COLOR:WrapTextInColorCode(DEFAULT) or bar.name,
			value = bar,
...
main.grab:SetPoint("TOPLEFT", main.addBtnFromDataBroker, "BOTTOMLEFT")
